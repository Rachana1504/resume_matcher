...
from extractors import (
    extract_text, extract_resume_data, extract_skills,
    normalize_skills, clean_entry_name
)
...
def _containment_match(jd_norm: set, res_norm: set) -> Tuple[set, set]:
    """
    Consider a skill matched if one normalized key *contains* the other
    (e.g., 'microsoftexcel' contains 'excel').
    """
    matched = set()
    for k in jd_norm:
        for r in res_norm:
...
def _jd_skill_sets(jd_entry: Dict[str, Any]):
    ...
    jd_map: Dict[str, str] = {}
    for s in base:
        norm = next(iter(normalize_skills([s])), s)
        jd_map.setdefault(norm, s)
    for k in jd_norm:
...
# ---------- Public API ----------
def match_resume_to_jds(resume_path: str, jd_cache: Dict[str, dict]):
    """
    Two-arg API expected by your FastAPI route.
    """
    ...
    for jd_name in jd_cache.keys():
        base = _compare(resume_embed, res_norm, resume_loc, jd_name, jd_cache.get(jd_name), resume_name)
        if not base:
            continue
        ...
        def periods(items):
            ...
            for e in items or []:
                ...
                end_dt = e[2]
                end = ("Present" if end_dt.year == 9999 or end_dt > datetime.now()
